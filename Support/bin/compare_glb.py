...
from tbdata.glb import BaseGlb  # pylint: disable=no-name-in-module,import-error
...
def first_glob(glob_pat):
    ...
    if len(maybe) == 0:
        raise LookupError("No %s" % glob_pat)
    if len(maybe) == 1:
        return maybe[0].replace("\\", "/")
...
def get_latest_glb(tilt_name, poly):
    ...
    return first_glob(
        os.path.join(os.path.join(ROOT, matches[-1]), directory, "*.glb*")
    )
...
def tweak_fix_sampler(dcts):
    for label, dct in enumerate(dcts):
        ...
        if label == 1:
            ...
            dct["samplers"] = dict(
                (rename(k), v) for (k, v) in list(dct["samplers"].items())
            )
...
def tweak_rename_refimage(dcts):
    ...
    for _, image in items(dcts[1]["images"]):
        if "uri" in image:
...
def compare_glb(
    ...
    glbs = [BaseGlb.create(x) for x in [a, b]]
    objs = [json.loads(g.get_json()) for g in glbs]
    for tweak in tweaks:
        tweak(objs)
    details = jsondiff.diff(
        objs[0],
        objs[1],
        syntax="symmetric",
        dump=True,
...
def compare_to_baseline(
    name, binary=True, poly=True, baseline_dir_name=DEFAULT_BASELINE_DIR
):
