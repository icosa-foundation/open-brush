...
configcopy = None
defaulttemplate = os.path.realpath(
    os.path.join(
        ...
        if line.startswith("TBProfile:"):
            if started:
                if line.startswith("TBProfile: END"):
                    ...
                    print(
                        (line[len("TBProfile: ") : -2])
                    )  # Strip off the CR LF at the end of each line
            else:
...
def create_config(sketchname, csv, template, lod):
    ...
    temp.write(
        configtemplate % (os.path.basename(sketchname), "true" if csv else "false", lod)
    )
    temp.close()
...
def upload_sketch(sketchname):
    ...
    subprocess.check_call(
        'adb push "%s" "%s"' % (sketchname, sketchFullPath), stdout=FNULL
    )
...
def profile_sketches(
    ...
    try:
        ...
        if csv:
            print(
                "Sketch, Num Frames, Min ms, Median ms, Max ms, StdDev, StdDev %, Batches, Triangles, 90fps+, 75fps, 60fps, Sub-60fps"
            )
...
def upload_config(configFile):
    subprocess.check_call(
        'adb push "%s" "%s"' % (configFile, tiltbrushcfgunescaped), stdout=FNULL
    )
...
def upload_config_text(configText):
    ...
    temp.close()
    subprocess.check_call(
        'adb push "%s" "%s"' % (temp.name, tiltbrushcfgunescaped), stdout=FNULL
    )
    ...
    try:
        ...
        for config in configs[skip:]:
            ...
            if configFile:
                ...
                if not sketch:
                    sys.stderr.write(
                        "Could not load %s" % (os.path.basename(match.group(1)))
                    )
                    continue
                clear_config()
...
def enable_powersaving(enable):
    try:
        subprocess.check_call(
            "adb shell pps-tool.sh %s" % ("enable" if enable else "disable"),
            stdout=FNULL,
        )
...
def save_config():
    ...
    os.remove(temp.name)
    subprocess.call(
        'adb pull "%s" "%s"' % (tiltbrushcfgunescaped, temp.name), stdout=FNULL
    )
...
def get_screenshot(filename):
    ...
    try:
        subprocess.check_call(
            'adb pull "%sTilt Brush/%s" sshot_%d_%s"'
            % (tbdir, screenshotFilename, screenshotnum, screenshotFilename),
            stdout=FNULL,
        )
        screenshotnum += 1
...
def main():
    ...
    parser.add_argument(
        "--kill", help="Kill an existing Tilt Brush process.", action="store_true"
    )
    parser.add_argument(
        "--getprofile", help="Look for and output profile data.", action="store_true"
    )
    parser.add_argument(
        "--quiet", help="Do not print info messages.", action="store_true"
    )
    parser.add_argument(
        "--clearconfig",
        help="Delete the Tilt Brush.cfg file from device.",
        action="store_true",
    )
    parser.add_argument(
        "--createconfig",
        metavar="SKETCHNAME",
        help="Create a new Tilt brush.cfg file suitable for profiling.",
        nargs=1,
    )
    parser.add_argument(
        "--uploadsketch",
        help="Upload a sketch to the device.",
        nargs=1,
        metavar="SKETCHNAME",
    )
    parser.add_argument(
        "--profilesketches", help="Profile a set of sketches.", nargs="*"
    )
    parser.add_argument(
        "--profilereps",
        help="Number of repetitions of a profile",
        nargs="?",
        default=1,
        metavar="REPETITIONS",
    )
    parser.add_argument(
        "--csv",
        help="Output profile information as csv.",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--uploadconfig",
        metavar="CONFIGFILE",
        help="Upload a config file to device",
        nargs=1,
    )
    parser.add_argument(
        "--profileconfigs",
        metavar="CONFIGFILES",
        help="Profile using a set of config files.",
        nargs="*",
    )
    parser.add_argument(
        "--powersaving", metavar="ON/OFF", help="Turn power saving on or off.", nargs=1
    )
    parser.add_argument(
        "--configtemplate",
        ...
        help="A template to use for the Tilt Brush.cfg",
    )
    parser.add_argument(
        "--lod",
        metavar="CONFIG TEMPLATE",
        nargs="?",
        default=99999,
        help="Global maximum level of detail.",
    )
    parser.add_argument(
        "--skip",
        metavar="ITEMS TO SKIP",
        nargs="?",
        default=0,
    ...
    if args.profilesketches:
        printInfo = False
        profile_sketches(
            expand_globs(args.profilesketches),
            int(args.profilereps),
            args.csv,
            args.configtemplate,
            args.lod,
            skip,
    ...
    if args.profileconfigs:
        printInfo = False
        profile_configs(
            expand_globs(args.profileconfigs), int(args.profilereps), args.csv, skip
        )
        return
    if args.powersaving:
