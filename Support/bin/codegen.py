...
def find_project_dir():
    def search_upwards_from(dirname):
        ...
        while True:
            if os.path.exists(os.path.join(dirname, "Assets")) and os.path.exists(
                os.path.join(dirname, "ProjectSettings")
            ):
                return dirname
...
def do_codegen(filename):
  ...
  \#endif \n""",
    ...
    def expand_all(match):
        ...
        body = re.sub(r"^\s*//.*\n", "", body, flags=re.M)
        expansion = "\n\n".join(
            expand_one(var_name, var_value, body) for var_value in var_values
        )
...
def main():
    ...
    files = [
        "Assets/Scripts/SketchBinaryWriter.cs",
