...
class ObfuscationSection:
    ...
    def add_entry(self, sym, ob):
        ...
        if sym in self.sym_to_ob:
            ...
            raise Exception(
                "Unexpected: %s -> %s and %s" % (sym, ob, self.sym_to_ob[sym])
            )
    ...
    def shorten(symbol):
        ...
        short = short.rsplit(".", 1)[
            -1
        ]  # Not QUITE sure about this, but let's see how it goes
        return short
...
class ObfuscationMap:
    ...
    def load_from_git_rev(self, git_object):
        ...
        if proc.returncode != 0:
            print(
                "WARN: Couldn't load deobfuscation from '%s'\n%s"
                % (git_object, stderr),
                file=sys.stderr,
            )
            return
...
def format_nicely(txt, verbose):
    ...
    if not verbose:
        ...
        lines = [
            remove_arglist(line)
            for line in lines
            if line != " (wrapper remoting-invoke-with-check)"
        ]
        ...
        def demangle_lambda(line):
            ...
            return re.sub(
                r"(?P<prefix>[a-zA-Z0-9_.]+)\+<>c\.<(?P<owner>[a-zA-Z0-9_]+)>b__(?P<id>[0-9_]+)",
                repl,
                line,
            )
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Add symbols from specified release branch",
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Do not elide any information"
    )
    ...
    if omap.is_empty():
        parser.error(
            "No symbols loaded. Do you need to pass '--release' or '--branch'?"
        )
