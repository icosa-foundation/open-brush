...
def make_steam_cmd(line, store):
    ...
    (?P<buildid>\d+)   \t
    (?P<date>[^\t]+)  \t
    Release \s (?P<version>\d+\.\d+)-(?P<sha>[0-9a-f]+)""",
        re.X,
    )
    m = pat.match(line)
    if m is None:
        raise Error(
            "Could not parse. Input should look something like\n%s" % TEST_STEAM_2
        )
    dct = m.groupdict()
    return [
        "git",
        "tag",
        "-m",
        "Steam build %s" % (dct["buildid"]),
        make_tag_name(dct["version"], store),
        dct["sha"],
    ]
...
def make_oculus_cmd(txt, store):
    ...
    return [
        ...
        "tag",
        "-m",
        "Oculus Version Code %s" % (dct["versioncode"]),
        make_tag_name(dct["version"], store),
        dct["sha"],
    ]
