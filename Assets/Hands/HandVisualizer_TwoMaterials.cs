...
namespace UnityEngine.XR.Hands.Samples.VisualizerSample
{
    ...
    public class HandVisualizer : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("(Optional) Depth-only material for the hand mesh. This should render first and write depth without writing color.")]
        Material m_HandMeshDepthMaterial;

        [SerializeField]
        [Tooltip("(Optional) Transparent visible material for the hand mesh. This should render after the depth material.")]
        Material m_HandMeshTransparentMaterial;
        ...
        class HandGameObjects
        {
            ...
            public HandGameObjects(
                ...
                Material meshTransparentMaterial,
                ...
                GameObject velocityPrefab)
            {
                ...
                // The hand mesh normally contains a single submesh. Assigning two materials
                // causes Unity to draw that submesh twice:
                //   1) depth-only pass/material
                //   2) visible transparent pass/material
                //
                // This gives the transparent hand a populated depth buffer first, preventing
                // fingers/palm surfaces behind the nearest hand surface from blending through it.
                if (meshDepthMaterial != null && meshTransparentMaterial != null)
                {
                    m_MeshController.handMeshRenderer.sharedMaterials = new[]
                    {
                        meshDepthMaterial,
                        meshTransparentMaterial
                    };
                }
                else if (meshDepthMaterial != null)
                {
                    m_MeshController.handMeshRenderer.sharedMaterial = meshDepthMaterial;
                }
                else if (meshTransparentMaterial != null)
                {
                    m_MeshController.handMeshRenderer.sharedMaterial = meshTransparentMaterial;
                }
