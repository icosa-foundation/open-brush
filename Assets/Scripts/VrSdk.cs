...
namespace TiltBrush
{
    ...
    public class VrSdk : MonoBehaviour
    {
        ...
        private ValveOpenXRRefreshRateFeature m_DisplayRefreshFeature;

        private void ApplyConfiguredDisplayRefreshRate()
        {
            if (m_DisplayRefreshFeature == null) return;
            var rates = new List<float>();
            if (m_DisplayRefreshFeature.EnumerateRefreshRates(ref rates) != 0) return;
            float requestedRate = App.UserConfig.Video.DisplayRefresh;
            foreach (float rate in rates)
            {
                if (Mathf.Abs(rate - requestedRate) >= 1f) continue;
                if (m_DisplayRefreshFeature.SetRefreshRate(rate) == 0)
                {
                    Debug.Log($"[PRIntegration90Hz] Set display refresh to {rate}");
                }
                break;
            }
        }
        ...
        void Start()
        {
            if (App.Config.m_SdkMode == SdkMode.UnityXR)
            {
                m_DisplayRefreshFeature = UnityEngine.XR.OpenXR.OpenXRSettings.Instance?
                    .GetFeature<ValveOpenXRRefreshRateFeature>();
                if (m_DisplayRefreshFeature != null && m_DisplayRefreshFeature.enabled)
                {
                    m_DisplayRefreshFeature.OnRefreshRateFeatureAvailable += ApplyConfiguredDisplayRefreshRate;
                    if (m_DisplayRefreshFeature.initialized) ApplyConfiguredDisplayRefreshRate();
                }
            }
        ...
        void OnDestroy()
        {
            if (m_DisplayRefreshFeature != null)
            {
                m_DisplayRefreshFeature.OnRefreshRateFeatureAvailable -= ApplyConfiguredDisplayRefreshRate;
            }
