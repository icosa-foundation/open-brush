            ...
            Vector3 end_RS)
        {
            ...
            if (m_IsCapturingForOwner)
            {
                ...
                return new ManualColocationCaptureFeedback
                {
                    CanConfirm = true,
                    Message = $"{string.Format(
                        Localize("MP_MANUAL_COLOCATION_REFERENCE_LENGTH"),
                        Vector3.Distance(start_RS, end_RS))}{controls}",
                };
            }
            ...
            string warning = result.Warnings == ManualColocationWarning.None
                ? string.Empty
                : $" {string.Format(
                    Localize("MP_MANUAL_COLOCATION_WARNING"),
                    result.Warnings)}";
            return new ManualColocationCaptureFeedback
            {
                CanConfirm = true,
                Message = $"{string.Format(
                    Localize("MP_MANUAL_COLOCATION_MEASURED_LENGTH"),
                    result.LocalLengthMeters)}{warning}{controls}",
            };
        }
