...
namespace TiltBrush
{
    ...
    public class SnipJoinTool : ToggleStrokeModificationTool
    {
        ...
        override protected void HandleIntersection(Stroke strokeB)
        {
            if (InputManager.m_Instance.GetCommand(InputManager.SketchCommands.Activate))
            {
                ...
                {
                    var splitPoint = ClosestControlPoint(strokeB);
                    if (splitPoint == 0 || splitPoint == strokeB.m_ControlPoints.Length - 1) return;
                    SketchMemoryScript.m_Instance.PerformAndRecordCommand(
                        new SnipStrokeCommand(strokeB, splitPoint)
                    );
        ...
        override public void AssignControllerMaterials(InputManager.ControllerName controller)
        {
            ...
            {
                InputManager.Brush.Geometry.ShowSnipJoinToggle(m_JoinModeOn);
            }
        }
    }
} // namespace TiltBrush
