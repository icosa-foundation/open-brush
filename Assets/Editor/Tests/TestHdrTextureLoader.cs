...
namespace TiltBrush
{
    internal class TestHdrTextureLoader
    {
        ...
        [Test]
        public void DecodesLegacyRadianceRuns()
        {
            byte[] header = Encoding.ASCII.GetBytes(
                "#?RADIANCE\nFORMAT=32-bit_rle_rgbe\n\n-Y 1 +X 5\n");
            byte[] sourcePixels =
            ...
            var decoded = HdrTextureLoader.Decode(bytes, "legacy-rle.hdr");

            Assert.AreEqual(5, decoded.Width);
            Assert.AreEqual(1, decoded.Height);
            Assert.AreEqual(Color.red, decoded.Pixels[0]);
            Assert.AreEqual(Color.red, decoded.Pixels[1]);
            Assert.AreEqual(Color.red, decoded.Pixels[2]);
            Assert.AreEqual(Color.blue, decoded.Pixels[3]);
            Assert.AreEqual(Color.blue, decoded.Pixels[4]);
        }
    }
}
