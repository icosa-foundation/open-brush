...
using UnityEngine;
#if USE_DOTNETZIP
using ZipSubfileReader = OpenBrush.TiltFile.ZipSubfileReader_DotNetZip;
using ZipLibrary = Ionic.Zip;
#else
using ZipSubfileReader = OpenBrush.TiltFile.ZipSubfileReader_SharpZipLib;
...
{
    /// <summary>
    /// Tilt file format - whether to save as ZIP or directory
    /// </summary>
    ...
    {
        /// <summary>
        /// Global setting for preferred Tilt file format. Can be overridden per-write.
        /// </summary>
        public static TiltFormat PreferredFormat { get; set; } = TiltFormat.Inherit;

        /// <summary>
        /// Logger for Tilt file operations. Defaults to NullLogger.
        /// </summary>
        public static ITiltFileLogger Logger { get; set; } = NullLogger.Instance;
        ...
        sealed public class AtomicWriter : IDisposable
        {
            ...
            public AtomicWriter(string path)
            {
                ...
                bool useZip;
                switch (TiltFile.PreferredFormat)
                {
            ...
            if (File.Exists(m_Fullpath))
            {
                try
                {
                    using (var stream = new FileStream(m_Fullpath, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        {
                            Logger.LogFormat("Bad .tilt sentinel or header: {0}", m_Fullpath);
                            return false;
                        }
                        if (header.headerSize < HEADER_SIZE)
                        {
                            Logger.LogFormat("Unexpected header length: {0}", m_Fullpath);
                            return false;
                        }
                        stream.Seek(header.headerSize - HEADER_SIZE, SeekOrigin.Current);
                        if ((new BinaryReader(stream)).ReadUInt32() != PKZIP_SENTINEL)
                        {
                            Logger.LogFormat("Zip sentinel not found: {0}", m_Fullpath);
                            return false;
                        }
                        return true;
                    }
                ...
                {
                    Logger.LogFormat("File does not have read permissions: {0}", m_Fullpath);
                    return false;
                }
                catch (IOException)
                {
