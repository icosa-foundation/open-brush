...
| 11 | P2 | Normalize video extension matching | Uppercase extensions behave like lowercase in discovery and updates | Discovery implemented; isolated checks passed; update filtering remains action 7; Unity validation pending |
...
6. Next priority: action 2, independent video restoration, followed by root-qualified model identity and asynchronous recovery. Actions 1, 5 and 11 have not been Unity/device tested; release gate 12 remains open. The reviewed checks can be rerun from machine-local `Logs/pr1120-plan-checks.ps1`. No fixes have been pushed during this plan execution.
