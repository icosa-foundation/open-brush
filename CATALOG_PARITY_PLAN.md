...
15. 2026-09-13: Action 7 video/sound update correction: changed paths obey direct-child extension filtering, intersect the current detected files, and deduplicate with platform-appropriate path comparisons. Existing changed entries are removed before replacements are added; changed+deleted files are not reintroduced. Video now disposes obsolete watchers, both catalogs ignore callbacks from previous watchers, and name/directory notifications are enabled. Playing videos are not disposed when removed from the catalog. Added NUnit filtering/change-set cases; Editor test project compiles. Native event and isolated scan validation follow. Commit message: `Filter and deduplicate local video and sound catalog updates`.
